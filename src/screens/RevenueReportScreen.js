...
function RevenueReportScreen(props) {
  ...
  const revenueReport=() =>{
    SuperAdminAPIService.getRevenueReport(selectedBus,selectedInterval)
    .then((res)=>{
      if( res.data.status == "success"){
            ...
            <div class="row">
              <div class="col-4 my-4">
                <select
                  ...
                  onChange={setBus}
                >
                  <option value="bus" hidden selected>Select Bus</option>
                  {bus.map((bus) => {
